...
db = RecipeDatabase(uri=MONGO_URI)
if not db.connect():
...
def unsave_recipe(recipe_id):
    db.db["saved_recipes"].delete_one({
        "user": session["username"],
        "recipe_id": ObjectId(recipe_id)
    })
...
def saved_recipes():
    saved_docs = list(db.db["saved_recipes"].find({"user": session["username"]}))
    saved_ids = [d["recipe_id"] for d in saved_docs]
    saved = list(db.collection.find({"_id": {"$in": saved_ids}}))
